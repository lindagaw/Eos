...
if __name__ == '__main__':
    ...
    # load models
    src_encoder = torch.nn.Sequential(*(list(progenitor.children())[3:10]))
    src_classifier = torch.nn.Sequential(*(list(progenitor.children())[11:]))

    tgt_classifier = torch.nn.Sequential(*(list(progenitor.children())[3:10]))
    tgt_encoder = torch.nn.Sequential(*(list(progenitor.children())[11:]))
