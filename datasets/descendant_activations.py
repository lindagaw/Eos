...
class Descendant_Activations(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, download=False, dataset='undefined'):
        ...
        if download:
            ...
            xs_train = np.load('snapshots//' + dataset + '_dev_1st_conv_activations.npy')
            xs_test = np.load('snapshots//' + dataset + '_eval_1st_conv_activations.npy')
            ys_train = np.load('snapshots//' + dataset + '_dev_1st_conv_activations_labels.npy')
            ys_test = np.load('snapshots//' + dataset + '_eval_1st_conv_activations_labels.npy')
